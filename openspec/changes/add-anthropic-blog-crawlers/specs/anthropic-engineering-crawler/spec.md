...
### Requirement: Crawl Anthropic Engineering blog posts
The system SHALL crawl https://www.anthropic.com/engineering and extract blog post metadata including title, URL, and publication date.

#### Scenario: Successful crawl of engineering blog
- **WHEN** the crawler runs against https://www.anthropic.com/engineering
...
### Requirement: Store engineering blog data
The system SHALL store crawled engineering blog posts in MongoDB collection `anthropic_engineering_blog` with deduplication by title.

#### Scenario: Insert new blog posts
- **WHEN** crawler finds new blog posts not in database
- **THEN** system inserts them into `anthropic_engineering_blog` collection
...
- **THEN** system logs "N anthropic engineering blog posts parsed"

#### Scenario: Log insertion count
- **WHEN** crawler completes database insertion
- **THEN** system logs "N anthropic engineering blog inserted"
...
- **WHEN** `anthropic_engineering_blog` is in `enabled_topics` list or `enabled_topics` is None
- **THEN** system runs the crawler

#### Scenario: Crawler disabled in config
- **WHEN** `anthropic_engineering_blog` is NOT in `enabled_topics` list
